...
Item {
    ...
    implicitWidth: cover.implicitWidth
        + Config.dashboard.sizes.mediaVisualiserSize * 2
        + details.implicitWidth
        + details.anchors.leftMargin
        + bongocat.implicitWidth
        + bongocat.anchors.leftMargin * 2
    ...
    property bool colorPickQueued: false

    function currentArtUrl(): string {
        return Players.active?.trackArtUrl ?? "";
    }

    function triggerColorPick(): void {
        const artUrl = root.currentArtUrl();
        if (!artUrl)
            return;

        if (colorPickProc.running) {
            colorPickQueued = true;
            return;
        }

        colorPickProc.exec({
            command: ["/home/hei/scripts/colorpick.py", artUrl]
        });
    }

    function scheduleColorPick(): void {
        triggerColorPick();
        colorPickDebounce.restart();
    }
    ...
    Process {
        ...
        onExited: code => {
            if (code !== 0)
                console.warn("COLOR: colorpick failed:", code);

            colorFile.reload();

            if (root.colorPickQueued) {
                root.colorPickQueued = false;
                root.triggerColorPick();
            }
        }
    }

    Timer {
        id: colorPickDebounce

        interval: 120
        repeat: false
        onTriggered: root.triggerColorPick()
    }

    FileView {
        id: colorFile
        path: "/tmp/cover_color.txt"
        preload: true
        watchChanges: true
        ...
        onLoaded: {
            ...
            if (c.startsWith("#"))
                ...
                console.log("COLOR: invalid value:", c);
        }

        onLoadFailed: err => console.warn("COLOR: load failed:", FileViewError.toString(err))
    }
    ...
    Component.onCompleted: scheduleColorPick()
    ...
    Shape {
        ...
        property color colour: root.accentColor
    ...
    Variants {
        ...
        ShapePath {
            ...
            readonly property real value: Math.max(
                0.02,
                Math.min(1, (Audio.cava.values[modelData] ?? 0) * 1.15)
            )
    ...
    Item {
        ...
        AnimatedImage {
            ...
            speed: Math.max(0.6, Math.min(2.0, (Audio.beatTracker.bpm ?? 120) / 190))
