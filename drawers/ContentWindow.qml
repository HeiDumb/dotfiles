...
StyledWindow {
    ...
    readonly property real borderThickness: 30 * (1 - fsTransitionProg)
    ...
    Item {
        ...
        Rectangle {
            anchors.fill: parent
            anchors.margins: Math.max(1, Math.round(root.borderThickness / 2))

            color: "transparent"
            radius: Math.max(0, root.borderRounding - anchors.margins)

            border.width: 3
            border.color: Qt.alpha(Colours.palette.m3primary, 0.8)
