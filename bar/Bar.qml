...
ColumnLayout {
    ...
    function shouldHoldTrayMenu(y: real): bool {
        const pad = Math.max(56, Config.border.rounding * 3);
        return Math.abs(y - popouts.currentCenter) <= pad;
    }
    ...
    function checkPopout(y: real): void {
        ...
        if (!ch) {
            if (currentIsTrayMenu && shouldHoldTrayMenu(y))
                return;
        ...
        if (id === "statusIcons" && Config.bar.popouts.statusIcons) {
            ...
            popouts.hasCurrent = true;
        } else if (currentIsTrayMenu && shouldHoldTrayMenu(y)) {
            return;
        } else if (currentIsTrayMenu) {
            popouts.hasCurrent = false;
            closeTray();
        }
    }
